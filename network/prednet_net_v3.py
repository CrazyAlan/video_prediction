...
"""Changed the Error layer from the subtraction to convolve the two estimation """
...
def init_A(img_height, img_width,stack_sizes, batch_size, nb_layers):
  batch_size = tf.to_int32(batch_size)
  init_A = []
  for l in range(nb_layers):
    factor = 2**l
    init_A.append(tf.Variable(tf.zeros([batch_size,img_height/factor,img_width/factor,stack_sizes[l]])))
  return init_A
...
def construct_model(images,
  ...
  A_lstm_state = nb_layers*[None]
  A_hidden = nb_layers*[None]

  # A = init_A(img_height, img_width,stack_sizes, batch_size, nb_layers)
  ...
  for image in images[:-1]:
    ...
    with slim.arg_scope(
      ...
      for l in range(nb_layers):
        if l == 0:
          ...
          a_up = slim.conv2d(A_hidden[l-1], stack_sizes[l], [A_filt_sizes[l-1], A_filt_sizes[l-1]] ,stride=1, scope='conv_a'+str(l))
        ...
        A_hidden[l] = tf_layers.layer_norm(A_hidden[l], scope='layer_norm_A'+str(l))


      # Update R states
      for l in reversed(range(nb_layers)):
        if l == nb_layers-1:
          hidden[l], lstm_state[l] = lstm_func(A_hidden[l], lstm_state[l], stack_sizes[l], scope='lstm'+str(l))
        else:
          ...
          feature = tf.concat([A[l], r_up], axis=3)
    ...
    # Update A_hat    
      net=slim.conv2d(hidden[0], hidden[0].get_shape()[3], [Ahat_filt_sizes[0],Ahat_filt_sizes[0]], stride=1, scope='conv'+str(0))
      net=tf.nn.relu(net - RELU_SHIFT) + RELU_SHIFT
      net=tf.minimum(net, tf.ones_like(net)*pixel_max)

      gen_images.append(net)
