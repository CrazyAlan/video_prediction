  ...
  for image in images[:-1]:
    ...
    with slim.arg_scope(
      ...
      for l in reversed(range(nb_layers)):
        if l == nb_layers-1:
          hidden[l], lstm_state[l] = lstm_func(A[l], lstm_state[l], stack_sizes[l], scope='lstm'+str(l))
        else:
      ...
      for l in range(nb_layers):
          ...
          # pdb.set_trace()
          if l < nb_layers-1:
            conv_A = slim.conv2d(A[l], stack_sizes[l+1], [A_filt_sizes[l], A_filt_sizes[l]] ,stride=1, scope='conv_e'+str(l))
            conv_A = tf.nn.relu(conv_A -  RELU_SHIFT) + RELU_SHIFT
            A.append(tf.nn.max_pool(conv_A, [1,2,2,1], [1,2,2,1],'SAME'))
